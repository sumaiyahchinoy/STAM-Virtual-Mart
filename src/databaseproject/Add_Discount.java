...
public class Add_Discount extends javax.swing.JFrame {
    ...
    private void addActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addActionPerformed
        ...
        if (discountName.getText().equals("")) {
            JOptionPane.showMessageDialog(f, "Missing Value, Enter Discount Name", "ERROR", JOptionPane.ERROR_MESSAGE);
        } else if (DiscountPercentage.getText().equals("")) {
            JOptionPane.showMessageDialog(f, "Missing Value, Enter Discount Percentage", "ERROR", JOptionPane.ERROR_MESSAGE);
        }else if (!isNumeric(DiscountPercentage.getText())) {
           JOptionPane.showMessageDialog(f, "Invalid Value, Enter Numeric Discount Percentage Value", "ERROR", JOptionPane.ERROR_MESSAGE);
        }  else {
            String percent = DiscountPercentage.getText();
            String name = discountName.getText();
            try {
                String Str_Query = "insert into discount(discount_name,discount_percent) values(?,?)";
                PreparedStatement ps = mycon.prepareStatement(Str_Query);
                ps.setString(1, name);
                ps.setInt(2, Integer.valueOf(percent));
                int k = ps.executeUpdate();
                if (k == 1) {
                    JOptionPane.showMessageDialog(this, "Discount Addded");
